 ...
 Employee ID :
  ...
  <TR>
    ...
    <TD vAlign=top align=middle width="907" height="109">
      <P><IMG height=84 src="../Image/engtmsatt.gif" width=683 border=0 ><BR><BR>
        <FORM name=frmEmpScore action=TD_UpdateEmpScore.asp method=post>
      <TABLE cellSpacing=0 width="100%" border=0>
        ...
        <tr><td width=10%></td>
        <td><FONT class=small><b>Current Position&nbsp;&nbsp;&nbsp;</b> </FONT>
			  <input type=text name=txtCPos readonly size=30 value="<%=tmpCurrentID%>" ><input type=hidden name=posID value="<%=trim(right(tmpCurrentID,20))%>">&nbsp;&nbsp;&nbsp;
        </td>
        </tr>
        <tr>
        <td>&nbsp;</td>
    ...
    <tr>
      ...
      <td bgcolor="#f3f3f3" width="150"><font class="marineblack">Employee Score</font></td>
            ...
            <%     page_size =10
			        ...
			        response.write "<td><input type=hidden name=txtRowNo value=" + cstr(rowno) + "><input type=hidden name=txtAction value=" + request.Form("HCP"+ cstr(row))+"></td>"
			        response.Write "</tr>"
			        rs.close
			        set rs = nothing
			        myconn.close
  ...
  ''''''''''''''''''''''''''''''''''''''''''''''paging records start'''''''''''''''''''''''''''''''''''''''''''''''''
			...
			iF current_page >= 2 then
				Response.Write "<a href=""TD_EmpScore.asp?page=1"
			...
			if current_page < page_count then
				Response.Write "<a href=""TD_EmpScore.asp?page="
