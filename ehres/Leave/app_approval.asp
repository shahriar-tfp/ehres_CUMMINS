				   ...
				   If vStatus = "P" then
			        ...
			        do while rs.AbsolutePage = current_page and not rs.EOF
			           'Do Until webdbRecordset.EOF
				           ...
				           response.write "<td align='left'" + colour + "><font class='small'>" + rs("LeaveType") + "<input type='hidden' name=L" + cstr(rowno) + " value= " + rs("leaveid") + "></td>"
...
'				   maxrow = approverow
		      ...
		      if approve = "true" then
		         response.redirect "/ehres/Leave/updateSuccess.asp"
		      end if
		    end if	
		%>
