 ...
 Employee ID :
  ...
  <TR>
    ...
    <TD vAlign=top align=middle width="907" height="109">
      <P><IMG height=84 src="../Image/enggapana.gif" width=683 border=0 ><BR><BR>
        <FORM name=frmGapAnalysis action=TD_EmpScore.asp method=post>
      <TABLE cellSpacing=0 width="100%" border=0>
        ...
        <td><FONT class=small><b>Current Position&nbsp;&nbsp;&nbsp;</b> </FONT>
			  <input type=text name=txtCPos readonly size=30 value="<%=tmpCurrentID%>" >&nbsp;&nbsp;&nbsp;
			  <% if tmpCurrentID <> "" then%>
				<input name=cmdTarget type=button value=Target onclick=GetTarget>
				<%end if%>
        </td>
        </tr>
        <tr>
    ...
    <tr>
      ...
      <td bgcolor="#f3f3f3" width="50"><font class="marineblack">Performance Management</font></td>
            ...
            <%     page_size =10
                    ...
                    if tmpCurrentID ="" then                
						 ssql = "Exec sp_Wtd_selGapAnalysis '" + Session("Regisno") + "','" + tmpEmpID + "','" + trim(right(tmpCurrentID,10)) + "','',0,'CURRENT_COMP'"
					else	
					     ssql = "Exec sp_Wtd_selGapAnalysis '" + Session("Regisno") + "','" + tmpEmpID + "','" + trim(right(tmpCurrentID,10)) + "','',0,'CURRENT_COMP'"
					end if
  ...
  ''''''''''''''''''''''''''''''''''''''''''''''paging records start'''''''''''''''''''''''''''''''''''''''''''''''''
			...
			iF current_page >= 2 then
				Response.Write "<a href=""TD_GapAnalysis.asp?page=1"
			...
			if current_page < page_count then
				Response.Write "<a href=""TD_GapAnalysis.asp?page="
