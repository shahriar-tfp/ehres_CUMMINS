 ...
 Employee ID :
      ...
      <tr>
            ...
            <select name=cboempid style="HEIGHT: 22px; LEFT:83px; TOP: 8px; WIDTH: 400px"  >  
                ...
                ssql ="Exec sp_Wls_selApprovalAuthority '" + Session("Regisno") + "','HRLS','','','" + trim(Session("EmpID")) + "','" + trim(Session("EmpID")) + "','BY_AUTHORITY'"
			 	...
			 	if Request.form("cboempid") = "" then 'or tmpEmpID = "1" then 
          ...
          <TD>
                ...
                <%end if %>   
                  ...
                  <% if Request.Form("txtdate1")="" then
                  	    response.write " value='" & session("ssDate1error") & "'"
                  	 'else
                  ...
                  <% if Request.Form("txtdate2")="" then
                  	    response.write " value='" & session("ssDate2error") & "'"
                  	 'else
  ...
  <TR>
    ...
    <% If request("cboErrCode") <> "" And request("cboErrCode") <> "ALL" Then %>
          ...
          <td width="70" bgcolor="#f3f3f3"><font class="marineblack"><b>EmpID</b></font></td>
	...
	<% End If%>
            ...
            <%      page_size =10
		            ...
		            rs.pagesize = page_size
  		           ...
  		           empid = session("ssempiderror")
  		           error = session("sserrorerror")
  		           date1 = session("ssdate1error")
  		           date2 = session("ssdate2error")
  		            		           
  		           'Response.Write(found) Request.Form("cboempid") <> "" or                                
  		        if  Request.Form("txtSearch") = "Search" then  
				...
				if rs.RecordCount > 0 then
				   ...
				   colour = 0
			        ...
			        'do until rs.EOF
			          ...
			          If (request("cboErrCode") <> "ALL" and request("cboErrCode") <> "") or (error <> "ALL" and error <> "") Then
			          'If error <> "ALL" and error <> "" Then
				         response.write "<tr>"
				      ...
				      ElseIf (request("cboErrCode") = "ALL" or request("cboErrCode") = "") or (error = "ALL" or error = "") Then
				      'ElseIf error = "ALL" or error = "" Then
				         response.write "<tr>"
				         response.write "<td></td>"
				         response.write "<td height='20' align='left'" + colour + "><font class='small'>" + rs("empid") + "</td>"
				         response.write "<td height='20' align='left'" + colour + "><font class='small'>" + rs("empname") + "</td>"
