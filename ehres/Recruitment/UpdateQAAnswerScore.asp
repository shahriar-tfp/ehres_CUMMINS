...
Response.Buffer = true
	   ...
	   if isnumeric(maxrow) then
		do until rowcount = cint(maxrow)
	      rowcount = rowcount + 1
			...
			ssql = "Exec sp_WRQ_insQA '" & Request.form("cboRQID") & "' , '" _
	      	            & request.Form("Eva" + cstr(rowcount)) & "','" _
	      	            & request.Form("S" + cstr(rowcount)) & "'," & request.Form("M" + cstr(rowcount)) & ",'DEL'"
			webdbCommand.CommandText = ssql
			webdb.Execute webdbCommand.CommandText
			
			ssql = "Exec sp_WRQ_insQA '" & Request.form("cboRQID") & "' , '" _
	      	            & request.Form("Eva" + cstr(rowcount)) & "','" _
	      	            & request.Form("S" + cstr(rowcount)) & "'," & request.Form("M" + cstr(rowcount)) & ",'ADD'"
			webdbCommand.CommandText = ssql
			webdb.Execute webdbCommand.CommandText
      ...
      response.redirect "RQ_ScoreMaintenance.asp"
